...
| 9a  | Para cada item de `blocking`, até 3 `revisor` em modo cético (`REFUTAR:`, lentes correção/reprodução/escopo); fica só com ≥2 de 3 não refutados; ≤ 9 céticos por PR (#146)                       | orquestrador   | fan-out bounded                    |
| 9b  | Comentar o veredito na PR (com `refutation: [{finding, votes}]` se houve 9a) e aplicar a label sobre o `blocking` que sobreviveu: `approved` → `review:approved`; `rejected` → `state:qa-failed` | orquestrador   | label = rastro do veredito         |

...
| 9   | Lançar o `revisor` (só leitura): AC × diff, escopo, controle negativo, invariantes, qualidade → JSON           | orquestrador → revisor | `review:approved` ou `state:qa-failed` |
...
| 12  | Merge que muda decisão, contrato ou comando → `documentador` abre PR `docs` (sem revisor)                      | documentador           | CI                                     |
...
`review:approved` só o revisor põe. `human` só sai quando a pessoa resolve.

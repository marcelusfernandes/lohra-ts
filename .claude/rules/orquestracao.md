...
| 6   | Gates locais (os cinco do CLAUDE.md) e `npm run prova -- <slug>`, e **dogfooding real** pelo gatilho por substância do passo 4 de `git-workflow.md` (senão `N/A` citando a chave tocada)                                                          | implementador  | —                                  |
| 7   | Push e PR pela skill `pr` (`Closes #N`, AC copiados, `state:in-review`). **Para. Nunca mergeia.**                                                                                                                                                 | implementador  | —                                  |
| 8   | CI: `checks (20)`, `checks (22)`, `provenance`; em PR também `escopo`, `contratos`, `controle-negativo` e `mutations` (fatias de mutação por path, #225)                                                                                          | GitHub Actions | required checks                    |
...
| 11  | Merge de risco (toca `src/state/`, `src/workflow/`, `.github/`, `package.json`) → `qa` roda suíte inteira + `npm run mutations:all` em worktree pinado; fatia que `mutations.yml` já rodou verde no HEAD mergeado é citada pelo run, não repetida | qa             | só reporta, nunca corrige          |

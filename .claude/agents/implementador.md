...
4. Escreva o teste que reprova. Módulo novo? Crie o arquivo com um **stub que lança**
   ...
   os testes **e** os stubs. É exatamente o que o check `controle-negativo` do CI
   verifica: existe um commit `test(red):` no range que toca os testes do diff e adiciona
...
6. Gates locais, **sempre os cinco do CLAUDE.md** (`npm run build` → `typecheck` →
   `lint` → `format:check` → `test`) e `npm run prova -- <slug>`, mesmo em issue só de
   docs — o repo tem testes que fixam prosa (`tests/t22-docs.test.ts`). Verde em todos.

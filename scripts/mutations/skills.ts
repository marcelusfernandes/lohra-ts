// Runner de mutação dos 12 mutantes de `src/skills/**` (issue #681). Molde
// de `doctor.ts`: único sandbox de `git archive` (via `prepareArchiveSandbox`
// do harness comum), um mutante de cada vez com
...
import { skillsMutants } from "./skills-mutants.js";
import type { Focus, MutantResult, MutationReport } from "./types.js";

const root = resolve(fileURLToPath(import.meta.url), "../../..");
const evidenceDirectory = resolve(root, ".mutation-evidence/skills");
const suite = "skills-mutations";
...
function runAllMutants(sandbox: string): {
  ...
  const files = [
    ...new Set(skillsMutants.flatMap((mutant) => mutant.edits.map((edit) => edit.file))),
  ];
  ...
  for (const mutant of skillsMutants) uniqueFoci.set(focusKeyOf(mutant.focus), mutant.focus);
  ...
  for (const mutant of skillsMutants) {

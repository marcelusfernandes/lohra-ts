// Catálogo puro dos 44 mutantes de `mutations:t15` (issue #186, follow-up
// da #154/PR #185). Extraído byte a byte de `workflow-executor.ts`
// (mesmos ids/before/after/focalTests — nenhum mutante muda) para que
// `tests/mutations-slices.test.ts` possa contar importando este módulo em
// vez de ler o texto fonte por regex: até aqui, `workflow-executor.ts` era
// catálogo E runner (chamava `main()` incondicionalmente na avaliação do
// módulo), então importá-lo disparava uma corrida de mutação real. Com a
// guarda de entry-point (`ehEntryPoint`, issue #186) no runner, o catálogo
// não precisa mais morar no mesmo arquivo — segue o padrão dos outros
// cinco catálogos declarativos (`*-mutants.ts`).
//
// Nada aqui depende de I/O nem importa `node:child_process`/`node:fs`: um
// módulo de dado puro, seguro para `import` estático.
import type { Edit } from "./types.js";

export const focalTests = [

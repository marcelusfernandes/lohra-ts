// Catálogo de 33 mutantes da fatia `supervision` (issue #451, milestone 14 —
...
//
// Issue #647 (grupo A de #637, itens 1 e 6): este arquivo tocou o teto de
// 800 linhas (PR #609/#627) e não tinha mais espaço para o próximo mutante
// da fatia. Os 8 mutantes de N1 a Y1 (abort em voo #519, `normalizeResumeId`
// #540, fold de faults #540 r2, achados 2/3 de #594) mudaram para o catálogo
// irmão `scripts/mutations/supervision-mutants-2.ts` (dado puro, mesmo
// molde) sem alterar `before`/`after`/`focus` de nenhum -- `supervision.ts`
// concatena os dois catálogos numa só corrida de `npm run
// mutations:supervision`, como `context-window.ts` já faz com

...
async function main(): Promise<number> {
  ...
  try {
    ...
    // summary.json: historicamente (harness de paridade, removido em #167)
    // summary.json era capturado inteiro e comparado por igualdade canônica
    // exata em dezenas de manifests — um campo a mais ali quebraria essa
    // comparação. Único jeito de um teste que roda o driver como processo
    // filho saber qual porta efêmera (config.port === 0) foi atribuída pelo
    // SO, sem depender de um número fixo e conhecido (issue #3).
    writeFileSync(

      ...
      try {
        ...
        oracleWs.sendText(
          JSON.stringify({
            ...
            params: { session_id: created.oracleSessionId },
          }),
        );
        candidateWs.sendText(
          JSON.stringify({
            ...
            id: 3,
            method: "session.history",
            params: { session_id: created.candidateSessionId },
          }),
        );
        const [oracleHistFrame, candidateHistFrame] = await Promise.all([
          oracleWs.nextFrame(10_000),
          candidateWs.nextFrame(10_000),
        ]);
        const oracleMessages = (
          JSON.parse(oracleHistFrame.payload.toString("utf8")) as {
            result: { messages: unknown[] };
          }
        ).result.messages;
        const candidateMessages = (
          JSON.parse(candidateHistFrame.payload.toString("utf8")) as {
            result: { messages: unknown[] };
          }
        ).result.messages;
        ...
        if (shapeDivergence !== null) {
          return divergent(id, `persisted shapes: ${shapeDivergence}`, {
            oracleShapes,
            candidateShapes,
          });
        }
        ...
        if (!roles.has("user") || !roles.has("tool")) {
          return divergent(id, "expected at least user and tool roles in the persisted history", {
            oracleShapes,
          });
        }
        const assistantShapes = oracleShapes
          .filter((shape) => shape.role === "assistant")
          .map((shape) => shape.keys);
        const hasToolShape = assistantShapes.some(
          (keys) => JSON.stringify(keys) === JSON.stringify(EXPECTED.assistantTool),
        );
        const hasPlainShape = assistantShapes.some(
          (keys) => JSON.stringify(keys) === JSON.stringify(EXPECTED.assistantPlain),
        );
        if (!hasToolShape || !hasPlainShape) {
          return divergent(
            id,
            "expected both assistant-with-tool_calls and assistant-plain shapes",
            { assistantShapes },
          );
        }
        ...
        if (
          userShape === undefined ||
          JSON.stringify(userShape.keys) !== JSON.stringify(EXPECTED.user)
        ) {
        ...
        if (
          toolShape === undefined ||
          JSON.stringify(toolShape.keys) !== JSON.stringify(EXPECTED.tool)
        ) {
        ...
        const oracleRestMessages = (
          JSON.parse(oracleRest.body.toString("utf8")) as { messages: unknown[] }
        ).messages;
        const candidateRestMessages = (
          JSON.parse(candidateRest.body.toString("utf8")) as { messages: unknown[] }
        ).messages;
        const oracleRestVsWs = compareMasked(oracleRestMessages, oracleMessages);
        if (oracleRestVsWs !== null)
          return divergent(id, `oracle REST != WS: ${oracleRestVsWs}`, {
            oracleRestMessages,
            oracleMessages,
          });
        const candidateRestVsWs = compareMasked(candidateRestMessages, candidateMessages);
        if (candidateRestVsWs !== null) {
          return divergent(id, `candidate REST != WS: ${candidateRestVsWs}`, {
            candidateRestMessages,
            candidateMessages,
          });
        }

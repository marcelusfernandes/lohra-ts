...
export const AUTH_HEADER_MATRIX_SCENARIOS: readonly NamedScenario[] = [
  {
    ...
    run: async (ctx) => {
      ...
      if (ctx.dashboardToken === undefined) {
        return divergent(
          id,
          "scenario requires a pinned dashboardToken (SECURE_PHASE_DASHBOARD_TOKEN) but ctx.dashboardToken is undefined",
        );
      }
      ...
      const variants: readonly Variant[] = [
        ...
        {
          name: "auth_duplicate_good_then_bad",
          headers: [
            [TOKEN_HEADER, t],
        ...
        {
          name: "auth_duplicate_bad_then_good",
          headers: [
            [TOKEN_HEADER, "wrong-token"],
            [TOKEN_HEADER, t],
          ],
        ...
        if (oracle.status !== candidate.status) {
          return divergent(
            ...
            `${variant.name}: oracle=${String(oracle.status)} candidate=${String(candidate.status)}`,
            {
              ...
              oracleBody: jsonBody(oracle),
              candidateBody: jsonBody(candidate),
            },
          );
        }
        results.push({
      ...
      if (viaQuery[0].status !== viaQuery[1].status) {
        return divergent(
          id,
          `auth_via_query_param: oracle=${String(viaQuery[0].status)} candidate=${String(viaQuery[1].status)}`,
        );
      }
      results.push({
        name: "auth_via_query_param",
        oracleStatus: viaQuery[0].status,
        candidateStatus: viaQuery[1].status,
      });

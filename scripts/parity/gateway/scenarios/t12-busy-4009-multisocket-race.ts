...
async function raceOneSide(
  port: number,
  fakeUpstream: ScenarioContext["fakeUpstream"],
): Promise<SideRaceResult> {
  ...
  try {
    ...
    const sessionId =
      (JSON.parse(createFrame.payload.toString("utf8")) as { result?: { session_id?: string } })
        .result?.session_id ?? "";
        ...
        losers.map(async (loser, index) => {
          loser.sendText(
            JSON.stringify({
              ...
              id: 10 + index,
              method: "prompt.submit",
              params: { session_id: sessionId, text: "should be busy" },
            }),
          );
...
export const BUSY_4009_MULTISOCKET_SCENARIOS: readonly NamedScenario[] = [
  {
    ...
    run: async (ctx) => {
      ...
      if (JSON.stringify(oracleShapes) !== JSON.stringify(candidateShapes)) {
        return divergent(
          id,
          `loser response shapes oracle=${JSON.stringify(oracleShapes)} candidate=${JSON.stringify(candidateShapes)}`,
          {
            oracleResult,
            candidateResult,
          },
      ...
      if (!oracleShapes.every((shape) => shape === expectedError)) {
        return divergent(
          id,
          `expected all 3 losers to get {code:4009,message:"session busy"}, got ${JSON.stringify(oracleShapes)}`,
        );
      }
      if (!oracleResult.winnerCompleted || !candidateResult.winnerCompleted) {
        return divergent(
          id,
          "expected the held turn to complete normally on both sides after release",
          { oracleResult, candidateResult },
        );
      }

...
export const UPSTREAM_ERROR_WARNING_SCENARIOS: readonly NamedScenario[] = [
  {
    ...
    run: async (ctx) => {
      ...
      try {
        ...
        oracleWs.sendText(JSON.stringify({ jsonrpc: "2.0", id: 2, method: "prompt.submit", params: { session_id: created.oracleSessionId, text } }));
        candidateWs.sendText(JSON.stringify({ jsonrpc: "2.0", id: 2, method: "prompt.submit", params: { session_id: created.candidateSessionId, text } }));

        const [oracleEvents, candidateEvents] = await Promise.all([drainUntilComplete(oracleWs), drainUntilComplete(candidateWs)]);
        ...
        if (JSON.stringify(oracleSequence) !== JSON.stringify(candidateSequence)) {
          return divergent(id, `event sequence oracle=${JSON.stringify(oracleSequence)} candidate=${JSON.stringify(candidateSequence)}`, {
            oracleEvents,
            candidateEvents,
          });
        }
        if (oracleSequence.includes("error")) {
          return divergent(id, "expected no separate error event, both sides emitted one", { oracleSequence });
        }
        ...
        const payload = oracleComplete?.payload as { status?: string; warning?: string; text?: string } | undefined;
        if (payload?.status !== "error" || !(payload.warning ?? "").includes("418") || !(payload.warning ?? "").includes(UPSTREAM_FAILURE_NONCE)) {
          return divergent(id, "message.complete did not carry the expected error/warning shape", { payload });
        }

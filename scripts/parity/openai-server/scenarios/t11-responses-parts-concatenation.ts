...
export async function run(
  ...
  const probe: ProbeRecord & { upstream: UpstreamRequestRecord[] } = {
    ...(await probeBoth("parts-concatenated", oracle, candidate, (apiKey) => postRequestLines(apiKey, body), body)),
  ...
  const comparison = compareRaw(probe.oracle, probe.candidate, {
    oracleBody: probe.oracle.body.replaceAll(/"id":"resp_[0-9a-f]{32}"/gu, '"id":"<ID>"').replaceAll(/"created_at":\d+/gu, '"created_at":0').replaceAll(/"id":"msg_resp_[0-9a-f]{32}"/gu, '"id":"<ITEM-ID>"'),
    candidateBody: probe.candidate.body.replaceAll(/"id":"resp_[0-9a-f]{32}"/gu, '"id":"<ID>"').replaceAll(/"created_at":\d+/gu, '"created_at":0').replaceAll(/"id":"msg_resp_[0-9a-f]{32}"/gu, '"id":"<ITEM-ID>"'),
  });
  ...
  return {
    projection: {
      probes: [record],
      normalizations: [
        { path: "/v1/responses", rule: "`id`/`created_at`/item `id` normalized before the bilateral body diff." },

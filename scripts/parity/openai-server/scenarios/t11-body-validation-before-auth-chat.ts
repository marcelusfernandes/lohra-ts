...
const CASES: readonly CaseSpec[] = [
  ...
  },
  {
    ...
    body: '{"model": "fake-model-a", "messages": [{"role": "user", "content": "SCEN:ok hi"}]}',
    withAuth: false,
    expectedStatus: 422,
  },
  {
    id: "messages-nao-lista",
    contentType: "application/json",
  ...
  },
  {
    id: "temperature-hot",
    contentType: "application/json",
    body: '{"model": "m", "messages": [{"role": "user", "content": "x"}], "temperature": "hot"}',
    withAuth: false,
    expectedStatus: 422,
  },
  {
    id: "stream-null",
    contentType: "application/json",
    body: '{"model": "m", "messages": [{"role": "user", "content": "x"}], "stream": null}',
    withAuth: false,
    expectedStatus: 422,
  },
  {
  ...
  {
    id: "ultima-role-nao-user-com-auth",
    contentType: "application/json",
    body: '{"model": "m", "messages": [{"role": "assistant", "content": "x"}]}',
    withAuth: true,
    expectedStatus: 400,
  },
];
...
export async function run(
  ...
  const expectedStatusById = new Map(
    CASES.map((testCase) => [testCase.id, testCase.expectedStatus]),
  );
  ...
  return {
    projection: {
      ...
      normalizations: [
        ...
        {
          path: "*",
          rule: "`date`/`server` headers dropped; header order not compared (assertion 9 pins body key order, not header order).",
        },
      ],
    },
    rawEvidence,

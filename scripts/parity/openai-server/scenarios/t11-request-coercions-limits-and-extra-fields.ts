...
export async function run(
  ...
  const probe1 = await probeBoth("empty-model-null-content", oracle, candidate, (apiKey) => postRequestLines("/v1/chat/completions", apiKey, emptyModelBody), emptyModelBody);
  const upstream1 = upstream.requests.slice(before1);
  rawEntries["empty-model-null-content"] = { request: probe1.request, oracle: probe1.oracle, candidate: probe1.candidate, upstream: upstream1 };
  const comparison1 = compareRaw(probe1.oracle, probe1.candidate, { oracleBody: normalizeIds(probe1.oracle.body), candidateBody: normalizeIds(probe1.candidate.body) });
  checks["emptyModelEchoedOk"] = probe1.oracle.body.includes('"model":""') && probe1.candidate.body.includes('"model":""');
  ...
  checks["extraFieldsNotForwardedOk"] = upstream1.every(
    (record) => !("top_p" in record.body) && !("n" in record.body) && !("tools" in record.body) && !("tool_choice" in record.body),
  );

  // 2. stream:"true" (string) coerces to true — a real SSE response.
  const stringStreamBody = JSON.stringify({ model: "m", messages: [{ role: "user", content: "SCEN:ok hi" }], stream: "true" });
  const probe2 = await probeBoth("stream-string-true", oracle, candidate, (apiKey) => postRequestLines("/v1/chat/completions", apiKey, stringStreamBody), stringStreamBody);
  rawEntries["stream-string-true"] = { request: probe2.request, oracle: probe2.oracle, candidate: probe2.candidate };
  checks["stringStreamCoercedOk"] =
    probe2.oracle.headers.some(([name, value]) => name === "content-type" && value.startsWith("text/event-stream")) &&
    probe2.candidate.headers.some(([name, value]) => name === "content-type" && value.startsWith("text/event-stream"));
  ...
  const probe3 = await probeBoth("chat-stream-options-yes", oracle, candidate, (apiKey) => postRequestLines("/v1/chat/completions", apiKey, usageYesBody), usageYesBody);
  rawEntries["chat-stream-options-yes"] = { request: probe3.request, oracle: probe3.oracle, candidate: probe3.candidate };
  checks["stringTruthyUsageEnabledOk"] = probe3.oracle.body.includes('"usage":') && probe3.candidate.body.includes('"usage":');
  ...
  const responsesExtraFieldBody = JSON.stringify({ model: "m", input: "SCEN:ok hi", stream: true, stream_options: { include_usage: true } });
  const before4 = upstream.requests.length;
  const probe4 = await probeBoth("responses-stream-options-ignored", oracle, candidate, (apiKey) => postRequestLines("/v1/responses", apiKey, responsesExtraFieldBody), responsesExtraFieldBody);
  const upstream4 = upstream.requests.slice(before4);
  rawEntries["responses-stream-options-ignored"] = { request: probe4.request, oracle: probe4.oracle, candidate: probe4.candidate, upstream: upstream4 };
  ...
  checks["responsesFrameCountBilateralOk"] = oracleFrameCount === candidateFrameCount && oracleFrameCount >= 4;
  const noSeparateUsageFrame = (body: string): boolean => !/"type":\s*"response\.[a-z_]*usage/u.test(body);
  checks["responsesNoSeparateUsageFrameOk"] = noSeparateUsageFrame(probe4.oracle.body) && noSeparateUsageFrame(probe4.candidate.body);
  checks["responsesEndsWithCompletedOk"] =
    /"type":\s*"response\.completed"/u.test(probe4.oracle.body) && /"type":\s*"response\.completed"/u.test(probe4.candidate.body);

  // 5. Default limit (no max_tokens) uses the profile default 8192; absent
  // temperature omits the key entirely (never sends null).
  const defaultsBody = JSON.stringify({ model: "m", messages: [{ role: "user", content: "SCEN:ok hi" }] });
  const before5 = upstream.requests.length;
  const probe5 = await probeBoth("defaults-max-tokens-temperature", oracle, candidate, (apiKey) => postRequestLines("/v1/chat/completions", apiKey, defaultsBody), defaultsBody);
  const upstream5 = upstream.requests.slice(before5);
  rawEntries["defaults-max-tokens-temperature"] = { request: probe5.request, oracle: probe5.oracle, candidate: probe5.candidate, upstream: upstream5 };
  checks["defaultMaxTokensOk"] = upstream5.every((record: UpstreamRequestRecord) => record.body["max_tokens"] === 8192);
  checks["temperatureOmittedNotNullOk"] = upstream5.every((record: UpstreamRequestRecord) => !("temperature" in record.body));

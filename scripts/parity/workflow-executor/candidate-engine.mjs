...
class RulesRuntime {
  ...
  collect(id) {
    ...
    else if (role === "judge.attempt")
      output = this.fail === "judge-winner" ? "BEST-CANDIDATE" : "draft";
    else if (this.fail === "judge-invalid" && role === "judge.review")
    ...
    else if (this.fail === "gate-invalid" && role === "gate.reviewer")
      output = '{"unexpected":true}';
...
const successSpecs = {
  ...
  parallel: {
    meta: { name: "parallel" },
    nodes: [{ id: "n", type: "parallel", branches: ["a", "b"] }],
  },
  pipeline: {
    meta: { name: "pipeline" },
    nodes: [{ id: "n", type: "pipeline", items: ["a", "b"], stages: [{ prompt: "pipe ${item}" }] }],
  },
  verify: {
    meta: { name: "verify" },
    nodes: [
      { id: "n", type: "verify", finding: "claim", skeptics: 1, kill_if_majority_refute: true },
    ],
  },
  judge_panel: {
    meta: { name: "judge" },
  ...
  gate: {
    meta: { name: "gate" },
    nodes: [
      { id: "n", type: "gate", body: { prompt: "draft" }, validator: "approve", attempts: 1 },
    ],
  },
  completeness_check: {
    meta: { name: "completeness" },
    nodes: [{ id: "n", type: "completeness_check", task: "task", results: ["result"] }],
  },
  checkpoint: {
    meta: { name: "checkpoint" },
    nodes: [{ id: "n", type: "checkpoint", prompt: "approve?" }],
  },
};
...
async function runOne(raw, options = {}) {
  ...
  return {
    value: project(await engine.run(spec(raw), options.args ?? {}), runtime),
    runtime,
    engine,
  };
}
...
const schemaRetry = await runOne(
  {
    meta: { name: "schema" },
    nodes: [
      { id: "n", type: "agent", prompt: "x", schema: { type: "object", required: ["value"] } },
    ],
  },
  { fail: "schema" },
);
...
const nullResult = await new WorkflowEngine({ runtime: nullRuntime }).run(
  spec({
    meta: { name: "null-upstream" },
    nodes: [
      { id: "a", type: "agent", prompt: "x", retries: 0 },
      { id: "b", type: "agent", prompt: "${a}" },
    ],
  }),
);
...
const engineFault = project(
  await faultEngine.run(
    spec({
      meta: { name: "engine-fault" },
      nodes: [
        { id: "bad", type: "agent", prompt: "x" },
        { id: "good", type: "agent", prompt: "y" },
      ],
    }),
  ),
  faultRuntime,
...
const pipelinePreflight = await runOne(
  {
    meta: { name: "pipeline-preflight" },
    nodes: [
      { id: "n", type: "pipeline", items: ["a", "b", "c"], stages: [{ prompt: "pipe ${item}" }] },
    ],
  },
...
const gateSequential = await runOne(successSpecs.gate, {
  budget: new Budget({ tokenBudget: 100 }),
});
const judgeInvalid = await runOne(
  {
    meta: { name: "judge-invalid" },
    nodes: [{ id: "n", type: "judge_panel", attempts: ["attempt"], judges: 1, synthesize: null }],
  },
  { fail: "judge-invalid" },
);
const gateInvalid = await runOne(
  {
    meta: { name: "gate-invalid" },
    nodes: [
      { id: "n", type: "gate", body: { prompt: "draft" }, validator: "approve", attempts: 1 },
    ],
  },
...
const nanBudget = new Budget({
  poolWidth: Number.NaN,
  maxFanout: Number.NaN,
  lifetime: Number.NaN,
});
const anchorSchema = {
  $defs: { p: { $anchor: "positive", type: "integer", minimum: 1 } },
  $ref: "#positive",
};
const dynamicSchema = {
  $defs: { p: { $dynamicAnchor: "node", type: "object" } },
  $dynamicRef: "#node",
};
const embeddedIdSchema = {

...
import { durableFromRow, pauseFields, ownershipLost } from "../../../dist/workflow/service.js";
...
step(
  "write_owner_live",
  repository.putRunState("run-1", {
    name: "n",
    owner: a.holder,
    status: "running",
    pauseReason: null,
    pausePayloadJson: null,
    specJson: JSON.stringify({ meta: { name: "n" } }),
    argsJson: "{}",
    tokenBudget: null,
    tainted: false,
    progressJson: null,
...
step(
  "spend_put",
  repository.putRunSpend("run-1", 100, 10, 5, 0, 0, 0, {
    fence: Number(locks.runFenceOf("run-1")),
    holder: b.holder,
    now: CLOCK.now,
  }),
);
...
step(
  "cache_put_owned",
  repository.putCacheCell("run-1", "h1", "node", "{}", "complete", {
    fence: fenceNow,
    holder: b.holder,
    now: CLOCK.now,
  }),
...
step(
  "cache_put_stale",
  repository.putCacheCell("run-1", "h2", "node", "{}", "complete", {
    fence: fenceNow - 1,
    holder: b.holder,
    now: CLOCK.now,
  }),
...
step(
  "cancel_missing",
  stranger.holder === "proc-a3"
    ? repository.getRunState("nope") === null
      ? "missing"
      : "cancelled"
    : "?",
);
...
step(
  "is_stale_running_no_lease",
  statusAfter === "running" && locks.runLeaseExpiry("run-1", CLOCK.now) !== null && viewB !== null
    ? pauseFields(viewB) === null
    : false,
);
...
step(
  "h1_write_owned",
  repository.putRunState("run-h1", {
    name: "h",
    owner: "proc-h",
    status: "running",
    pauseReason: null,
    pausePayloadJson: null,
    specJson: JSON.stringify({ meta: { name: "h" } }),
    argsJson: "{}",
    tokenBudget: null,
    tainted: false,
    progressJson: null,

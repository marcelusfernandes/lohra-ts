...
| `T22-script-omitted`                        | `package.json` (script de segurança do T22 com prefixo `probe`) | Verifica que o inventário de scripts de paridade (`run-closeout.ts --check-only`) rejeita um script de closeout ausente — o próprio inventário some com #8.                                                                                                                                                                                                                                                                                       |
...
| `T22-t13-nested-t10-timeout-wiring`         | `scripts/parity/orchestration/run-gates.ts`                     | Idem: fiação do orçamento acima ao comando real de gates do T10 com prefixo `parity`.                                                                                                                                                                                                                                                                                                                                                             |
...
- `scripts/parity/closeout/inventory.json:56,66` continuam apontando
  ...
  e o comando de mutações do T20 com prefixo `parity` para
  `scripts/parity/web-tools/run-mutations.ts`
...
- **A** — exige o oráculo Python vivo para produzir um resultado que não seja
  ...
  arquivo cujo código-fonte cita as palavras `python`, `oracle`/`ORACLE`, o
  nome da variável de ambiente do workspace do oráculo, o nome do venv do
  oráculo, `python_runner` ou `sitecustomize` (nomes exatos: histórico em
  `docs/parity-validation.md`)
  ...
  A variável de ambiente do workspace do oráculo aparecia em 5 arquivos, não
  6 como no comentário do #8 (`grep -rl <nome-da-variável> scripts/parity`,
  árvore apagada em #167 — não reproduz mais a partir do HEAD atual).
...
- **48 scripts com prefixo `parity`** → a contagem desses scripts em
  `package.json`, na época, era **47**.
- **A variável de ambiente do workspace do oráculo em 6 arquivos** → a busca
  por essa variável em `scripts/parity` dava **5** (`closeout/run-closeout.ts`,
  `media/run-all.ts`, `orchestration/run-gates.ts`, `resolve.ts`,
  `state/probe-utils.mjs`).

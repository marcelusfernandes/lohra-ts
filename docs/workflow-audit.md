...
`AUDIT_POLICY` (`audit-model.ts:482-488`), devolvida em toda página de
consulta:
...
desliga (`auditEnabled`, `audit-model.ts:490-507`, lido em
...
`audit-model.ts:509-527`, lido no construtor de `AuditRepository`,
...
Toda gravação carrega `publicAuditIdentity` (`audit-model.ts:461-480`):
...
`PATH_FIELDS`/`node_path` em `safeValue`, `audit-model.ts:365`) aparece
...
último elemento (`audit-model.ts:472`).
...
| `audit.truncated`   | `publicAuditEvent` (`audit-model.ts:425-459`)                                                                                                                                                                                                       | evento serializado > 2048 bytes (`AUDIT_EVENT_BYTES`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `leaf.completed`    | `close()` (`audit-runtime.ts:317-344`), chamado de `collect()` (`audit-runtime.ts:468-504`)                                                                                                                                                         | `collect` volta `status: "complete"`; `payload.partial: true` (nunca `false`) quando o turno concluído absorveu ao menos um steer que interrompeu uma chamada em voo (D2/#520, M16-S5) — o `usage` já inclui essa parcela ESTIMADA (`ConversationRuntime.runTurn`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `leaf.failed`       | `close()` (`audit-runtime.ts:317-344`), chamado de `collect()` (`audit-runtime.ts:468-504`)/`cancel()` (`audit-runtime.ts:506-525`)                                                                                                                 | `collect` falha/cancela/estoura timeout, ou `cancel()` — sempre exatamente um evento terminal por `sub_id`; carrega `payload.error_kind` (vocabulário em "Vocabulário de falhas" abaixo) quando `collected.errorKind` não é `null`/`undefined` — inclusive `error_kind: "cancelled"` para TODO `leaf.failed` que `cancel()` produz, mesmo sem um `ChildResult` real assentado (`failedPayload(null, "cancelled")`, #517); `payload.partial: true` (nunca `false`) e `payload.usage` com uma estimativa (nunca uma medição) quando a folha foi abortada em voo (M16-S3/#518) — `usage_uncertain: true` sempre acompanha `partial: true`, mas o inverso não vale (um `leaf.failed` sem chamada em voo pode ter `usage_uncertain: true` e nenhum `partial`)                                                                                                                                                                                                                         |
| `leaf.steered`      | `auditedChildRuntime.steer`, dentro de `deliverSteer` (`audit-runtime.ts:410-442`, #423)                                                                                                                                                            | grava só quando o core ACEITA o steer (#444) — `queued: true` (fila) ou a ressurreição `{queued: false}` sem `refused` (novo turno), o que inclui um retry de schema que roda depois do `leaf.completed`/`leaf.failed` do próprio leaf (`engine.ts:296-312` — o leaf já reportou `status: "complete"` antes do engine checar o schema); nunca grava para `refused: "steer_cap"` nem para um `sub_id` terminal/desconhecido (`outcome === null`); `payload.source` é `engine` (todo `steer()` interno do engine, default) ou `operator` (4º argumento de `runtime.steerOutcome` — `workflow_steer`, `src/workflow/steer-tool.ts:285`, é o chamador desde #424); `payload.message_chars` é o único dado do texto do steer — metadata-only, nunca o texto; `attempt` é o do `spawn` original, não um índice de retry; `payload.interrupted: true` (nunca `false`, `audit-runtime.ts:437`) quando o steer também tirou de voo uma chamada de provedor em andamento (D2/#520, M16-S5) |
...
| `cancelled`       | `leaf.failed` de `cancel()` (`audit-runtime.ts:506-525`) — SEMPRE, mesmo sem `ChildResult` real assentado (`failedPayload(null, "cancelled")`, `audit-runtime.ts:126-131`); também de `collect()`'s branch `status === "cancelled"` quando o próprio `ChildResult.errorKind` já é `"cancelled"` (folha abortada em voo, M16-S3/#518) — produzido em `audit-runtime.ts`/`child-runner.ts`, não por `classifyProviderError` (ganhou produtor em #517, épico #490; antes deste ganhar produtor era reservado sem uso, como `timeout`/`context_length` abaixo) |
...
`sandbox_denied`, `timeout` e `context_length` completam os nove de
`classifyProviderError` — reservados para reuso futuro sem produtor hoje:
`timeout` para a folha, `context_length` para o classificador de janela, e
`sandbox_denied` nomeado assim (não `sandbox_refused`) por decisão do épico
#396. `sandbox_denied` também nomeia o campo `reason` de `tool.completed` na
tabela acima — outra allow-list fechada, `SAFE_STRING_VALUES.reason`
(`audit-model.ts`), não a mesma lista de `error_kind`; mesma palavra, dois
campos distintos.
...
Três marcadores booleanos que #517/#518/#520 (ADR 0005) acrescentaram a
`BOOLEAN_FIELDS` (`audit-model.ts:95-105`) — cada um só ever `true`, nunca
`false`: a chave simplesmente não é escrita quando não se aplica (mesma
convenção de `forced_fallback` acima, sem o `false` inventado).

| campo             | onde aparece                            | o que marca                                                                                                                                                                                                                                                                                           |
| ----------------- | --------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `partial`         | `leaf.failed`, `leaf.completed`         | o `usage` do mesmo payload inclui uma parcela ESTIMADA (`estimatePartialUsage`, `token-estimate.ts`), nunca uma medição real do provedor — uma folha abortada em voo (`leaf.failed`, M16-S3/#518) ou um turno que absorveu um steer-interrupt e ainda assim completou (`leaf.completed`, M16-S5/#520) |
| `usage_uncertain` | `leaf.failed`, `leaf.completed`         | acompanha TODO `partial: true` (nunca o contrário sozinho não implica `partial`) — o `usage` do payload não é uma medição confiável, mesmo que os números não sejam zero                                                                                                                              |
| `interrupted`     | `leaf.steered` (`audit-runtime.ts:437`) | o steer não só entregou um novo prompt (`queued`/ressurreição) — também abortou uma chamada de provedor que a folha tinha em voo naquele momento (M16-S5/#520)                                                                                                                                        |

`usage` sob `partial: true` nunca é `undefined` (`estimatePartialUsage`
sempre devolve os cinco campos de `Usage`, mesmo que zerados) — o
`usagePayload` comum (linha 100 de `audit-runtime.ts`) só omite `usage`
quando o `ChildResult` original não carrega nenhum, o que nunca é o caso de
uma folha abortada em voo.

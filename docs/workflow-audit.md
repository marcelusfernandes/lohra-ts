...
— tratados abaixo junto com o resto de cada tabela.
...
`AUDIT_POLICY` (`audit-model.ts:430-436`), devolvida em toda página de
consulta:
...
desliga (`auditEnabled`, `audit-model.ts:438-453`, lido em
`service.ts:412`); qualquer outro valor reconhecido (`on`/`1`/`true`/`yes`,
...
`audit-model.ts:457-474`, lido no construtor de `AuditRepository`,
...
Toda gravação carrega `publicAuditIdentity` (`audit-model.ts:409-425`):
...
`PATH_FIELDS`/`node_path` em `safeValue`, `audit-model.ts:312-313`) aparece
só dentro do `data.node_path` de `leaf.started` (`payload.node_path:
cc.nodePath`, `audit-runtime.ts:273`) — não em `identity`, que guarda só o
último elemento (`audit-model.ts:418`).

## Tabela de eventos

Vinte e um tipos formam a allow-list (`SAFE_EVENT_TYPES`,
`audit-model.ts:102-129`); um `event_type` fora dela vira
...
| `node.paused`       | `announceNodePaused` (`audit-producers.ts:255`)                                                                                                                                                                        | run pausado; `payload.reason` ∈ `checkpoint`, `quota_exhausted`, `route_fault` (5º valor, #426/M10-S5), `token_budget_exhausted`, `user_requested` — nunca duplica nem substitui `workflow.node`/`workflow.fault`                                                                                                                                                                                                                                                                                                                                                                    |
...
| `audit.truncated`   | `publicAuditEvent` (`audit-model.ts:373-403`)                                                                                                                                                                          | evento serializado > 2048 bytes (`AUDIT_EVENT_BYTES`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `leaf.started`      | `auditedChildRuntime.spawn` (`audit-runtime.ts:260-266`)                                                                                                                                                               | um `ChildRuntime.spawn` bem-sucedido                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `leaf.completed`    | `close()`, chamado de `collect()` (`audit-runtime.ts:230-257,283-321`)                                                                                                                                                 | `collect` volta `status: "complete"`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `leaf.failed`       | `close()`, chamado de `collect()`/`cancel()` (`audit-runtime.ts:230-257,283-327`)                                                                                                                                      | `collect` falha/cancela/estoura timeout, ou `cancel()` — sempre exatamente um evento terminal por `sub_id`; carrega `payload.error_kind` (vocabulário em "Vocabulário de falhas" abaixo) quando `collected.errorKind` não é `null`/`undefined`                                                                                                                                                                                                                                                                                                                                       |
| `leaf.steered`      | `auditedChildRuntime.steer` (`audit-runtime.ts:353-370`, #423)                                                                                                                                                         | qualquer `steer` num leaf que este decorador já viu nascer, mesmo depois do seu `leaf.completed`/`leaf.failed` (retry de schema, `engine.ts:304-308` — o leaf já reportou `status: "complete"` antes do engine checar o schema); `payload.source` é `engine` (todo `steer()` interno do engine, default) ou `operator` (4º argumento do decorador — `workflow_steer`, `src/workflow/steer-tool.ts:180`, é o chamador desde #424); `payload.message_chars` é o único dado do texto do steer — metadata-only, nunca o texto; `attempt` é o do `spawn` original, não um índice de retry |
| `tool.started`      | `auditedToolDispatch` (`audit-runtime.ts:131`)                                                                                                                                                                         | toda chamada de tool feita por um leaf                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| `tool.completed`    | `auditedToolDispatch` (recusa síncrona, `audit-runtime.ts:177-183`) · `close()` (dispatch ainda aberto ao fechar o leaf, `audit-runtime.ts:239-248`) · `onToolSettled` (assentamento real, `audit-runtime.ts:413-431`) | `{status:"error", reason:"sandbox_denied"}` — recusa síncrona do sandbox; `{status:"error", reason:"cancelled"}` — o leaf fechou (cancel, shutdown ou timeout) com esse dispatch ainda em voo, emitido por `close()` **antes** do evento terminal do leaf; `{status:"success"}`/`{status:"error"}` sem `reason` — assentamento real                                                                                                                                                                                                                                                  |
...
segundo registro, `identities` (`audit-runtime.ts:224`), populado junto com
...
- Uma tool call cujo assentamento real (`onToolSettled`) chega **depois**
  ...
  antes do terminal do leaf (`audit-runtime.ts:230-257`, #378);
  `onToolSettled`, ao rodar depois, encontra `open.get(leaf.subId)`
  `undefined` e não grava nada — nem decrementa nada
  (`audit-runtime.ts:413-431`). O par nunca fica órfão (sempre há um

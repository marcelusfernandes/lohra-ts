...
| `leaf.steered`      | `auditedChildRuntime.steer` (`audit-runtime.ts:332-350`, #423)                                                                                                                                             | qualquer `steer` num leaf que este decorador já viu nascer, mesmo depois do seu `leaf.completed`/`leaf.failed` (retry de schema, `engine.ts:304-308` — o leaf já reportou `status: "complete"` antes do engine checar o schema); `payload.source` é `engine` (default, todo caller hoje) ou `operator` (4º argumento do decorador, ainda sem chamador — tool de S3); `payload.message_chars` é o único dado do texto do steer — metadata-only, nunca o texto; `attempt` é o do `spawn` original, não um índice de retry |

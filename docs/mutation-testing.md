...
| `supervision`         | `mutations:supervision` |       19 | `supervision-mutants.ts`                                                                                              |

Total: 246. Os 12 mutantes de `workflow-durability-guard.ts` são
...
treze catálogos de dado puro estaticamente e prova essa soma (246) a cada
...
5. `npm test` roda `tests/mutations-slices.test.ts`, que reprova de duas
   ...
   total (246 + o novo) contra os treze catálogos importados, e a linha do
   ...
   `246` e a linha do catálogo em `CONTAGEM_POR_CATALOGO`) precisam de
...
`context-window` 15, `auth-mutants` 13, `supervision-mutants` 19, soma 246) e
a soma de 246 contra os treze catálogos importados; e que todo diretório de

...
export interface CreateSessionInput {
  ...
  /** Issue #586 (épico #575): a plain string (every caller today) persists
   * exactly as before -- only `system_prompt` gets written, the three band
   * columns stay NULL. The three-band form ALSO writes the flattened text
   * into `system_prompt` (via `systemPromptText`, same join rule
   * `SystemPromptSnapshot.text` uses) so a reader that only knows the
   * single column -- there are several: `getSession`/`listSessions`/the
   * gateway session list -- keeps working unchanged. */
  readonly systemPrompt?: string | SystemBands | null;
...
export class SessionRepository {
  ...
  public createSession(input: CreateSessionInput): void {
    const bands =
      typeof input.systemPrompt === "object" && input.systemPrompt !== null
        ? input.systemPrompt
        : null;
    const flattened =
    ...
    this.database
      .prepare(
        `INSERT INTO sessions
         (id, source, model, system_prompt, system_prompt_stable, system_prompt_context,
          system_prompt_volatile, parent_session_id, started_at, cwd, title)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(
        input.id,
  ...
  /** Issue #586 (épico #575): restores the prompt-caching bands for a
   * session, tolerant of a row written before this column existed --
   * `system_prompt_stable IS NULL` (every pre-#586 row, and every row
   * created from a plain string) means "no bands were ever split out",
   * never "read the row and find out it's broken": the whole flattened
   * `system_prompt` becomes `stable`, `context`/`volatile` empty. `null`
   * only when the session itself doesn't exist. */
  public systemPromptBands(id: string): SystemBands | null {
    const row = this.database
      .prepare(
        "SELECT system_prompt, system_prompt_stable, system_prompt_context, system_prompt_volatile FROM sessions WHERE id = ?",
      )
      .get(id) as Row | undefined;
    if (row === undefined) return null;
    if (typeof row.system_prompt_stable === "string") {
      return {
        stable: row.system_prompt_stable,
        context: typeof row.system_prompt_context === "string" ? row.system_prompt_context : "",
        volatile: typeof row.system_prompt_volatile === "string" ? row.system_prompt_volatile : "",
      };
    }
    return {
      stable: typeof row.system_prompt === "string" ? row.system_prompt : "",
      context: "",
      volatile: "",
    };
  }

  /** Issue #587 AC: persists a title generated by the profile's
   * `defaultAuxModel` (`AuxClient.title`) -- `title TEXT` already exists on
   * `sessions` (no migration), and `listSessions`/`getSession` already
   * SELECT it, so `session_search browse` picks this up with no change of
   * its own (`SessionSearchTool` forwards `SearchRepository.listSessions()`
   * verbatim). Never throws on a missing id -- an UPDATE matching zero rows
   * is a no-op, same as calling this after the session's already gone. */
  public setTitle(id: string, title: string): void {
    this.database.prepare("UPDATE sessions SET title = ? WHERE id = ?").run(title, id);
  }

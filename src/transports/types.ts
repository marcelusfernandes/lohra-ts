 ...
 * call/session) uncached. Correction (#608, achado da #624 sobre a PR #621):
 * a transport that doesn't understand it (chat-completions, responses) DOES
 * receive this shape today -- `chat.ts`/`dashboard.ts` pass the full
 * `SystemPromptSnapshot` since #586's 2ª rodada, regardless of which
 * transport ends up handling the call -- `BuildKwargsOptions.system` is
 * `SystemBands` in practice for them too; each flattens it via
 * `systemPromptText` (`src/transports/chat-completions.ts`,
 * `src/transports/responses.ts`) rather than caching a breakpoint. */

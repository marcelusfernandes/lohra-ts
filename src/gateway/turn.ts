import { publicError } from "../commands/chat.js";
...
// status/canary preserved via the same publicError() chat.ts already uses
// (L21/assertion 53) -- never a raw thrown exception, and never silently
// swallowed.
export async function driveGatewayTurn(input: {
  ...
  try {
    ...
    return { status: "error", warning: publicError(error) };
  }
}

...
- **`sandbox_refusals`** counts tool calls the sandbox denied inside a leaf —
  ...
  allowlist, or, once the run is tainted, EVERY `read_file`/`write_file`
  and EVERY `web_fetch`/`web_search` the leaf calls — even a path inside
  scope or a host on the allowlist stops counting as an exception once
  tainted. It is cumulative: a resume adds the new stretch's
